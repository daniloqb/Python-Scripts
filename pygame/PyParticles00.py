...
def combine(p1, p2):
    if math.hypot(p1.x - p2.x, p1.y - p2.y) < p1.size + p2.size:
        ...
        p1.mass += p2.mass
        p2.combine = True
        p1.size = calculateRadius(p1.mass)
        return p2
...
class Particle:
    ...
    def __init__(self, name,(x, y), size, mass=1):
        self.name = name
        self.x = x
        self.y = y
        self.size = size
...
class Environment:
    ...
    def update(self):
        ...
        '''
        temp = self.particles
        for i,particle in enumerate(temp):
            particle.move()
            for particle2 in temp[i+1:]:
               remove_particle = combine(particle,particle2)
